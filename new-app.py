...
------------------------
Analyzes titles, meta descriptions, and H1 tags for patterns, duplicates, 
...
def analyze_content(df, content_type, output_dir):
    """Analyze content (titles, meta descriptions, or H1 tags) across page types."""
...
def analyze_seo_content(df, output_dir):
    ...
    # Analyze titles, meta descriptions, and H1 tags
    logger.info("\nAnalyzing titles...")
    ...
    logger.info("\nAnalyzing H1 tags...")
    h1_duplicates, h1_pagetype_summary = analyze_content(df, 'metadata-h1-contents', output_dir)
    ...
    with open(report_path, 'w') as f:
        ...
        # H1 statistics
        f.write("H1 Tag Analysis\n")
        f.write("--------------\n")
        f.write(f"Total H1 tags: {len(df):,}\n")
        f.write(f"Unique H1 tags: {df['metadata-h1-contents'].nunique():,}\n")
        duplicate_h1s = len(df[df.duplicated(subset=['metadata-h1-contents'], keep=False)])
        f.write(f"Duplicate H1 tags: {duplicate_h1s:,} ({(duplicate_h1s/len(df)*100):.1f}%)\n")

def main():
    """Main execution function."""
    parser = argparse.ArgumentParser(
        description='Analyze SEO content including titles, meta descriptions, and H1 tags'
    )
    ...
    if headers:
        ...
        if response.lower() != 'y':
            ...
            h1_col = input("Enter the column name for H1 tags (or press Enter to skip): ")
            ...
            if h1_col: column_mapping['metadata-h1-contents'] = h1_col
    ...
    required_columns = {'Full URL', 'pagetype', 'Title', 'Meta Description', 'metadata-h1-contents'}
    ...
    try:
        ...
        logger.info("- metadata_h1_contents_duplicates_summary.csv")
        logger.info("- metadata_h1_contents_pagetype_summary.csv")
        ...
        logger.info("- metadata_h1_contents_2gram_analysis.csv")
        logger.info("- metadata_h1_contents_3gram_analysis.csv")
